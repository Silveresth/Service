...
self.addEventListener('push', (event) => {
  let data = { title: 'Service Market', message: 'Nouvelle notification !' };
  try {
    if (event.data) {
    ...
    if (event.data) {
      data = { title: 'Service Market', message: event.data.text() };
    }
  }
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'Service Market', options)
  );
});
