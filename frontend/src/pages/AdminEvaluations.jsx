...
export default function AdminEvaluations() {
  ...
  const loadEvaluations = async () => {
    ...
    setError(null);
    try {
      // Essai endpoint admin dédié, fallback sur l'endpoint général
      let res;
      try {
        res = await api.get('/admin/all_evaluations/');
      } catch {
        res = await api.get('/evaluations/');
      }
      const data = res.data?.results ?? res.data ?? [];
      setEvaluations(Array.isArray(data) ? data : []);
    } catch (err) {
      setError(err.response?.data?.detail || 'Erreur lors du chargement des évaluations');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setDeleteError('');
    try {
      // Essai endpoint admin dédié, fallback sur DELETE standard
      try {
  ...
  // Résout le nom du service quelle que soit la structure JSON reçue
  const getServiceNom = (e) =>
    e.service_nom || e.service?.nom || e.service_name || '-';

  // Résout le nom du client
  const getClientNom = (e) =>
    e.client_username || e.client?.username || e.client_nom || e.client || '-';

  // Résout le nom du prestataire
  const getPrestataireNom = (e) =>
    e.prestataire_username || e.prestataire?.user?.username ||
    e.prestataire?.username || e.prestataire || '-';

  const renderStars = (note) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
      {[1, 2, 3, 4, 5].map(s => (
        <i key={s}
          className={`bi bi-star${s <= note ? '-fill' : ''}`}
          style={{ color: s <= note ? '#f59e0b' : '#d1d5db', fontSize: '0.85rem' }}
        />
      ))}
      <span style={{ fontWeight: 700, color: '#f59e0b', fontSize: '0.82rem', marginLeft: 4 }}>
        {note}/5
      </span>
    </div>
  );
    ...
    <div className="admin-page">
      <div className="admin-container">
        <div className="admin-header">
          <h1><i className="bi bi-star"></i> Toutes les Évaluations</h1>
          <p>Surveillez les avis des clients</p>
        </div>
        <div className="admin-alert admin-alert-danger">
          <i className="bi bi-exclamation-triangle"></i>
          <span>{error}</span>
          <button onClick={loadEvaluations} className="btn btn-sm btn-outline-danger ms-auto">
            Réessayer
          </button>
        </div>
      </div>
    ...
    <div className="admin-page">
      <div className="admin-container">
        ...
        <div className="admin-header">
          ...
          <p>Consultez et modérez les avis clients — {evaluations.length} évaluation{evaluations.length !== 1 ? 's' : ''} au total</p>
        </div>
                  ...
                  <tbody>
                    {evaluations.map((e, idx) => (
                      <tr key={e.id}>
                        ...
                        <td>
                          <strong style={{ color: '#0c2340' }}>{getServiceNom(e)}</strong>
                        </td>
                        <td>
                          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span style={{
                              ...
                              {getPrestataireNom(e)[0]?.toUpperCase() || 'P'}
                            </span>
                            {getPrestataireNom(e)}
                          </span>
                        </td>
                        <td>
                          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span style={{
                              ...
                              {getClientNom(e)[0]?.toUpperCase() || 'C'}
                            </span>
                            {getClientNom(e)}
                          </span>
                        </td>
                        ...
                        <td>
                          <span style={{
                            color: e.commentaire ? '#374151' : '#9ca3af',
                            fontStyle: e.commentaire ? 'normal' : 'italic',
                            fontSize: '0.85rem',
                            display: '-webkit-box',
                            WebkitLineClamp: 2,
                        ...
                        <td style={{ fontSize: '0.8rem', color: '#6b7280', whiteSpace: 'nowrap' }}>
                          {e.created_at || e.date_eval
                            ? new Date(e.created_at || e.date_eval).toLocaleDateString('fr-FR', {
                        ...
                        <td>
                          <div className="table-actions">
                            <button
                              ...
                              onClick={() => setDeleteModal({ id: e.id, name: getServiceNom(e) })}
